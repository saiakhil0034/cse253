...
def unbalanced_sampler(dataset, cnt, emotions):
	# this ensures everyone has the same balanced subset for model training, don't change this seed value
	random.seed(20)
	e_counts = []
	print("\nBalanced Set:")
	...
	for e in emotions:
		balanced_subset[e] = copy.deepcopy(dataset[e][:])
		random.shuffle(balanced_subset[e])
		balanced_subset[e] = balanced_subset[e]
		print('{}: {} # of images'.format(e, len(balanced_subset[e])))
		e_counts.append(len(balanced_subset[e]))
	return balanced_subset, min_cnt, np.asarray(e_counts)
